...
/path/to/input_dir) is a programming language specializing
...
['omic data /path/to/input_dir), [machine
...
signif_results <- read.csv('hmp2_output/study.tsv',
sep='\t')
...
all_results <- read.csv('hmp2_output/study.tsv', sep='\t')
...
# Write results
write.table(all_results, 'hmp2_output/study.tsv', sep='\t')
...
unscaled_name <- system.file("extdata", "study.tsv",
                                package = "maaslin3")
...
```{r, echo = FALSE, cache = FALSE}
signif_results <-
read.csv('random_effects_output/study.tsv',
                            sep='\t')
...
| Scenario |$\geq4$ average observations per group|$<4$ average /path/to/input_dir
per group, metadata not collinear with the groups |$<4$ average /path/to/input_dir
...
Models differential prevalence | Models allowed | Accounts /path/to/input_dir
compositionality | Can use experimental absolute abundance information |
|:---:|:---:|:---:|:---:|:---:|:---:|
| MaAsLin 3 | Yes | Yes, for any model type | Any lme4 /path/to/input_dir
