...
write.table(data_in, file.path(output_tmp, 'study.tsv'), 
            row.names = T, sep = '\t', col.names=NA)
write.table(metadata, file.path(output_tmp, 'study.tsv'), 
            row.names = T, sep = '\t', col.names=NA)
write.table(covar_data, file.path(output_tmp, 'study.tsv'), 
            row.names = T, sep = '\t', col.names=NA)
write.table(unscaled, file.path(output_tmp, 'study.tsv'), 
            row.names = T, sep = '\t', col.names=NA)

data_in_new <- maaslin_read_data(input_data = file.path(output_tmp, 'study.tsv'), 
                input_metadata = file.path(output_tmp, 'study.tsv'), 
                feature_specific_covariate = file.path(output_tmp, 'study.tsv'), 
                unscaled_abundance = file.path(output_tmp, 'study.tsv'))
