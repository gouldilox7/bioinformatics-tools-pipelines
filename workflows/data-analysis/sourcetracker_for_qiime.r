...
# Rscript sourcetracker_for_qiime.r-t study.txt -m study.txt 
#
# run leave-one-out source-sample predictions using QIIME taxon abundance file:
# Rscript sourcetracker_for_qiime.r-t study.txt -m study.txt -s 
#
# run sink predictions using QIIME OTU table:
# Rscript sourcetracker_for_qiime.r-i study.txt -m study.txt 
#
# run sink predictions using QIIME OTU table with 1000 burnins, 25 random restarts, and rarefaction depth of 100:
# Rscript sourcetracker_for_qiime.r-i study.txt -m study.txt -b 1000 -n 25 -r 100< sourcetracker_for_qiime.r 
#
# run sink predictions using QIIME taxon abundance file and and input file listing the sampleids to predict:
# Rscript sourcetracker_for_qiime.r-t study.txt -m study.txt -f study.txt 
#
...
sink(sprintf('%s/study.txt',arglist[['-o']]))
...
if(!is.null(resultsfile)){
    ...
    if(tune.alphas.ntrials > 0){
        ...
        # save alphas
        sink(sprintf('%s/study.txt', outdir))
...
# save results file
sink(sprintf('%s/study.txt', outdir, filebase))
cat('SampleID\t')
...
if(!arglist[['--suppress_full_results']]){
    ...
    for(i in 1:length(results$train.envs)){
        ...
        filename.fractions <- sprintf('%s/study.txt', subdir, filebase, env.name)
...
save.mapping.file(results, map,
        filename=sprintf('%s/study.txt',outdir),
