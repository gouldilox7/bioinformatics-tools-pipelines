    ...
    for oligo in oligos:
        ...
        for sample in samples:
            ...
            if oligo in samples_dict[sample]:
                percent_abundances.append((samples_dict[sample][oligo] * 100.0 / /path/to/input_dir
...
def mapping_file_simple_check(mapping_file_path, samples_expected = None):
    ...
    for line in mapping_file.readlines():
        ...
        if len(fields) != len(header_fields):
            raise ConfigError("Not every line in the mapping file has the same number of fields " /path/to/input_dir
