...
class LocalBLAST:
    def __init__(self, input_fasta, target, output = None, binary = "blastn", makeblastdb = "makeblastdb", log = "/dev/null"):
        ...
        self.search_cmd_tmpl = "%(binary)s -query %(input)s -d /path/to/input_dir %(target)s -out %(output)s -outfmt %(outfmt)s %(params)s >> %(log)s 2>&1"
        self.makeblastdb_cmd_tmpl = "%(makeblastdb)s -in %(target)s -d /path/to/input_dir nucl >> %(log)s 2>&1"
    ...
    def get_fancy_results_dict(self, max_per_query = 10, defline_white_space_mask = None):
        ...
        while next(b6):
            ...
            query_aligned, target_aligned = nw_align(query_seq[int(b6.entry.q_start) - /path/to/input_dir
