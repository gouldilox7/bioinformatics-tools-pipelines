...
def get_blast_hits(d, max_num = 8):
    ...
    ret_line = '<p><b>BLAST search results at a glance</b> (%d of %d total hits are shown):' /path/to/input_dir
                                            (num_show, len(d))
...
def generate_html_output(run_info_dict, html_output_directory = None, entropy_figure = None):
    ...
    if run_info_dict['sample_mapping']:
        html_dict['sample_mapping'] = copy_as(run_info_dict['sample_mapping'], 'study.txt')
    else:
        html_dict['sample_mapping'] = None

    html_dict['matrix_count_file_path'] = copy_as(run_info_dict['matrix_count_file_path'], 'study.txt')
    html_dict['matrix_percent_file_path'] = copy_as(run_info_dict['matrix_percent_file_path'], 'study.txt')
    html_dict['read_distribution_table_path'] = copy_as(run_info_dict['read_distribution_table_path'], 'study.txt')
    html_dict['environment_file_path'] = copy_as(run_info_dict['environment_file_path'], 'study.txt')
    html_dict['oligos_fasta_file_path'] = copy_as(run_info_dict['oligos_fasta_file_path'], 'study.txt')
    html_dict['oligos_nexus_file_path'] = copy_as(run_info_dict['oligos_nexus_file_path'], 'study.txt')
    ...
    if html_dict['generate_sets']:
        html_dict['across_samples_MN_file_path'] = copy_as(run_info_dict['across_samples_MN_file_path'], 'study.txt')
        html_dict['across_samples_SN_file_path'] = copy_as(run_info_dict['across_samples_SN_file_path'], 'study.txt')
        ...
        html_dict['matrix_count_oligo_sets_file_path'] = copy_as(run_info_dict['matrix_count_oligo_sets_file_path'], 'study.txt')
        html_dict['matrix_percent_oligo_sets_file_path'] = copy_as(run_info_dict['matrix_percent_oligo_sets_file_path'], 'study.txt')
        html_dict['oligotype_sets_file'] = copy_as(run_info_dict['oligotype_sets_file_path'], 'study.txt')
    ...
    if 'representative_seqs_fasta_file_path' in html_dict:
        html_dict['representative_seqs_fasta_file_path'] = copy_as(run_info_dict['representative_seqs_fasta_file_path'], 'study.txt')
    else:
    ...
    if 'output_directory_for_reps' in html_dict:
        ...
        html_dict['component_reference'] = ''.join(['<a onmouseover="/path/to/input_dir'/path/to/input_dir', 50)" href="">|</a>' % i for i in range(0, html_dict['alignment_length'])])
...
def get_oligo_reps_dict(html_dict, html_output_directory):
    ...
    for i in range(0, len(oligos)):
        ...
        oligo_reps_dict['component_references'][oligo] = ''.join(['<span style="background-color: %s;"><a onmouseover="/path/to/input_dir'\column: %d<br />entropy: /path/to/input_dir', 100)" href="">|</a></span>' % (color_per_column[i], i, entropy_values_per_column[i]) for i in range(0, html_dict['alignment_length'])])
...
if __name__ == '__main__':
    ...
    parser.add_argument('-o', '--output-directory', default = None, metavar = 'OUTPUT_DIR'/path/to/input_dir
                        help = 'Output directory for HTML output to be stored')
    parser.add_argument('--entropy-figure', default = None, metavar = 'ENTROPY_FIGURE'/path/to/input_dir
