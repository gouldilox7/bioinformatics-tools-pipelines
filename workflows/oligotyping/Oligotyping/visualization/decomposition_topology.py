...
def topology_graph(topology_dict_path, match_levels = False):
            ...
            if node['type'] == 'node':
                ...
                while 1:
                    if label[0] == '0':
                        ...
                        break
                G.add_edge(node_id, node['parent'], size = int(node['size']), label = /path/to/input_dir
                           image = node['freq_curve_img_path'] if 'freq_curve_img_path' in node else /path/to/input_dir
    ...
    for node_id in nodes['root']['children']:
        node = nodes['root']
        G.add_edge('root', node_id, size = int(nodes['root']['size']), label = 'root'/path/to/input_dir
                           image = node['freq_curve_img_path'] if 'freq_curve_img_path' in node else /path/to/input_dir
                           final_node = False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('topology_dict', metavar = 'TOPOLOGY'/path/to/input_dir
    ...
    parser.add_argument('--title', default = None, metavar = 'TITLE'/path/to/input_dir
