...
def generate_default_figures(_object):
    ...
    for (analysis, script, output_dir) in [('Stackbar', 'study.R', 'stackbar')]:
    ...
    for (analysis, script, output_dir) in [('Read Distribution Lines', 'study.R', 'lines'),
                                           ('Read Distribution Bars', 'study.R', 'bars')]:
        figures_dict['basic_reports'][output_dir] = {}
    ...
    if not _object.skip_basic_analyses:
        for (analysis, script, output_dir) in [('Cluster Analysis', 'study.R', 'cluster_analysis'),
                                               ('NMDS Analysis', 'study.R', 'nmds_analysis')]:
...
def generate_exclusive_figures(_object):
    ...
    for category in sample_mapping_dict:
        ...
        mapping_file_path = get_temporary_file_name('%s-' % category, 'study.txt', _object.tmp_directory)
        ...
        if samples == _object.samples:
            ...
            matrix_percent_path = get_temporary_file_name('%s-' % category, 'study.txt', _object.tmp_directory)
            matrix_count_path = get_temporary_file_name('%s-' % category, 'study.txt', _object.tmp_directory)
        ...
        for (analysis, script, output_dir) in [('NMDS Analysis', 'study.R', 'nmds_analysis')]:
            ...
            for (distance_metric, matrix_file) in [("canberra", matrix_percent_path),
                ...
                output_prefix = os.path.join(target_dir, distance_metric)
                cmd_line = ('%s -o "%s" -d "/path/to/input_dir" -m "%s" --title "%s" "%s" "%s" >> "%s" 2>&1' % 
                                        (script,
                                         output_prefix,
                                         distance_metric,
                                         category,
        ...
        # heatmap
        for (analysis, script, output_dir) in [('Heatmap Analysis', 'study.R', 'heatmap_analysis')]:
            exclusive_figures_dict[category][output_dir] = {}
            ...
            for (distance_metric, matrix_file) in [("canberra", matrix_percent_path),
                                                   ...
                                                   ("bray", matrix_percent_path)]:
                output_prefix = os.path.join(target_dir, distance_metric)
                cmd_line = ('%s "%s" -m "%s" -d /path/to/input_dir --title "%s" -o "%s" >> "%s" 2>&1' % 
                                        (script,
                                         matrix_file,
                                         mapping_file_path,
                                         distance_metric,
