...
option_list <- list(
		...
		make_option(c("--output_directory"), default=".",
				help = "Output directory to store images [default \"/path/to/input_dir"]"),
		make_option(c("--remove_outliers"), default=F,
				help = "Remove upper outliers for better scaling [default \"/path/to/input_dir"]"),
		make_option(c("--pdf_height"), default=9,
				help = "PDF output height [default \"/path/to/input_dir"]"))


parser <- OptionParser(usage = "study.R [options] input_matrix sample_mapping", option_list=option_list,
