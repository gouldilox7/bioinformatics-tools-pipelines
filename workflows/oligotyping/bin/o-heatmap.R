...
option_list <- list(
		...
		make_option(c("-o", "--output_file_prefix"), default="unknown",
				help = "Output file prefix for visualization files [default \"/path/to/input_dir"]"),
		make_option(c("-d", "--distance_col"), default="horn",
				help = "Distance metric for columns [default \"/path/to/input_dir"]"),
		make_option(c("-r", "--distance_row"), default="horn",
				help = "Distance metric for rows [default \"/path/to/input_dir"]"),
		make_option(c("-c", "--clustering"), default="ward",
				help = "Clistering method [default \"/path/to/input_dir"]"),
		make_option(c("--pdf_height"), default=9,
				help = "PDF output height [default \"/path/to/input_dir"]"),
		make_option(c("--treeheight_col"), default=100,
				help = "Dendrogram size for columns [default \"/path/to/input_dir"]"),
		make_option(c("--treeheight_row"), default=100,
				help = "Dendrogram size for rows (0 would make it disappear) [default \"/path/to/input_dir"]"),
		make_option(c("--show_rownames"), default=F,
				help = "Show row names [default \"/path/to/input_dir"]"),
		make_option(c("-s", "--scale_the_other_way"), default=F,
				help = "Scale based on columns [default \"/path/to/input_dir"]"),
		make_option("--title", default="(unknown title)",
				help="Title for the output figure [default '%default']")
)

parser <- OptionParser(usage = "study.R [options] input_file", option_list=option_list,
